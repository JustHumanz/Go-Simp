...
const (
	ServiceName = config.YoutubePastTrackerService
)
...
func StartCheckYT(Group database.Group) {
	...
	if Group.YoutubeChannels != nil && *agency {
		for _, YtChan := range Group.YoutubeChannels {
			...
			Yt, err := Group.GetYtLiveStream(config.PastStatus, "")
			if err != nil {
				log.Error(err)
			}
			...
			for _, YoutubeCache := range Yt {
				if YoutubeCache.ID != 0 {
					...
					if Items.Snippet.VideoStatus == config.UpcomingStatus && YoutubeData.Status == config.PastStatus {
						log.WithFields(log.Fields{
